...
const capabilityMatrix = {
  ...
  "Roofer": ["roof-cover", "roofing", "waterproofing"],
...
function generateStageTasks(stageId) {
  const taskMap = {
    ...
    foundation: [
      { id: "site-clearing", name: "Site Clearing", requiredContractor: "foundation specialist", critical: false },
      { id: "site-layout", name: "Site Layout & Marking", requiredContractor: "surveyor", critical: false },
      { id: "excavation", name: "Excavation", requiredContractor: "excavation specialist", critical: false },
      ...
      { id: "backfilling", name: "Backfilling & Compaction", requiredContractor: "foundation specialist", critical: false }
    ],
    ...
    windows: [
      { id: "window-install", name: "Window Installation", requiredContractor: "carpenter", critical: false },
    ...
    finishing: [
      ...
      { id: "fixtures", name: "Fixtures", requiredContractor: "carpenter", critical: false }
    ]
  };
...
function renderCurrentActionButton() {
  ...
  if (nextTask.status === "active") {
    ...
    btn.onclick = () => openTaskModal(stage.id, nextTask.id);
  } else {
    btn.textContent = "Start Task";
    btn.onclick = () => startTask(stage.id, nextTask.id);
  }
...
function renderCurrentActionCard() {
  ...
  if (insightList) {
    ...
    insights.forEach(insight => {
      if (insight.type === "action") return; // 🚫 REMOVE actions from insights
  ...
  insights.forEach(insight => {
    ...
    if (insight.type === "action" && insight.role) {
      ...
      text.addEventListener("click", () => {
        routeToContractors(insight.role);
      });
    } else {
      text.textContent = insight.message;
    }
...
function generateRecommendations(state) {
  ...
  console.log("🔍 HIRED MAP:", state.hiredContractors);
  console.log("🔍 MATCH:", state.hiredContractors?.[required]);
  const hired = state.hiredContractors?.[required];
  ...
  if (!hired && nextTask) {
    actions.push({
      ...
      state.hiredContractors = state.hiredContractors || {};
      state.hiredContractors[role] = true;
...
contractorArray.forEach(item => {
  const { card, score, result } = item;

  card.classList.remove("hidden", "best-match");
  ...
  // ✅ Trust Score (keep your existing logic)
  ...
  if (empty) {
    ...
    empty.textContent = "No matching contractors available. Try another category or add one.";
  }
}
...
function initHireButton(contractor) {
  ...
  btn.onclick = () => {
    ...
    // ✅ Ensure global projectState exists
    if (!projectState) {
    ...
    const role = resolveRole(contractor.role);

    // ✅ Ensure structure exists
    projectState.hiredContractors = projectState.hiredContractors || {};

    // ✅ Save contractor under correct role
    projectState.hiredContractors[role] = contractor;

    // ✅ Persist using your correct pipeline
    saveProjectState();

    console.log("✅ Contractor saved:", role, projectState.hiredContractors);
